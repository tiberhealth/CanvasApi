...
using ListOptions = CanvasApi.Client.Pages.Models.ListOptions;
...
internal class PageApiClient : ApiClientBase, IPageApiClient
{
    ...
    public Task<IPage> Create(long id, Action<IWikiPage> pageFactory, CourseGroupFlag type = CourseGroupFlag.Course) =>
        this.ProcessWiki(HttpMethod.Post, $"/api/v1/{this.Api(type)}/{id}/pages", pageFactory);

    public async Task<IPage> Delete(long id, string url, CourseGroupFlag type = CourseGroupFlag.Course) =>
        await this.ApiClient.ApiOperation<Page>(HttpMethod.Delete, $"/api/v1/{this.Api(type)}/pages/{url}");
    ...
    public async Task<IPage> FrontPage(long id, CourseGroupFlag type = CourseGroupFlag.Course) =>
        await this.ApiClient.ApiOperation<Page>(HttpMethod.Get, $"/api/v1/{this.Api(type)}/front_page");
    ...
    public async Task<IEnumerable<IPage>> List(long id, Action<IListOptions> optionsFactory = null, CourseGroupFlag type = CourseGroupFlag.Course)
    {
        ...
        return await this.ApiClient.PagableApiOperation<Page, IListOptions>(HttpMethod.Get, $"/api/v1/{this.Api(type)}/{id}/pages", listOptions);
    }

    public async Task<IEnumerable<IPageRevision>> ListRevisions(long id, string url, CourseGroupFlag type = CourseGroupFlag.Course) =>
        await this.ApiClient.PagableApiOperation<PageRevision>(HttpMethod.Get, $"/api/v1/{this.Api(type)}/{id}/pages/{url}/revisions");

    public async Task<IPageRevision> Revert(long id, string url, long revisionId, CourseGroupFlag type = CourseGroupFlag.Course) =>
        await this.ApiClient.ApiOperation<PageRevision>(HttpMethod.Post, $"/api/v1/{this.Api(type)}/{id}/pages/{url}/revisions/{revisionId}");

    public async Task<IPage> Show(long id, string url, CourseGroupFlag type = CourseGroupFlag.Course) =>
        await this.ApiClient.ApiOperation<Page>(HttpMethod.Get, $"/api/v1/{this.Api(type)}/{id}/pages/{url}");
    ...
    {
        var apiUrl = $"/api/v1/{this.Api(type)}/{id}/pages/{url}/revisions/{revisionId?.ToString() ?? "latest"}";
    ...
    public Task<IPage> UpdateFontPage(long id, Action<IWikiPage> pageFactory, CourseGroupFlag type = CourseGroupFlag.Course) =>
        this.ProcessWiki(HttpMethod.Put, $"/api/v1/{this.Api(type)}/{id}/front_page", pageFactory);
   

    public Task<IPage> UpdatePage(long id, string url, Action<IWikiPage> pageFactory, CourseGroupFlag type = CourseGroupFlag.Course) =>
        this.ProcessWiki(HttpMethod.Put, $"/api/v1/{this.Api(type)}/{id}/pages/{url}", pageFactory);
